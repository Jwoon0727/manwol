...
function MonthItem({
  ...
  return (
    ...
    >
      ...
      >
        <span className="font-bold text-sm w-8 shrink-0 text-white transition-colors duration-200 group-hover:text-[#FFD061]">
          {item.month}월
        </span>
        <span
          className={`inline-flex items-center justify-center px-2 py-0.5 rounded-full text-xs font-bold shrink-0 transition-colors duration-200 ${
            item.highlight
              ? "bg-white text-[#111111] group-hover:bg-[#FFD061] group-hover:text-[#111111]"
              : "bg-white/8 text-white border border-white/30 group-hover:text-[#FFD061] group-hover:border-[#FFD061]/50"
          }`}
        >
          {item.ganzhi}
        </span>
        ...
        {open ? (
          ...
          <ChevronDown className="w-4 h-4 text-white shrink-0 transition-colors duration-200 group-hover:text-[#FFD061]" />
        )}
      </button>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          <div className="px-5 pb-4 pt-2 flex flex-wrap gap-2">
            {item.tags.map((tag) => (
              <span
                key={tag}
                className="px-3 py-1 rounded-full text-[11px] font-medium border border-white/30 text-white"
                style={{
